...
function loadFromURL(e) {
  ...
  var introspect = async o => {
    var u = makeIntrospectionQuery(o);
    var l = await d.query(u, {});
    try {
      if (l.error) {
        throw l.error;
      } else if (l.data) {
...
var L = "undefined" != typeof require ? require.resolve.bind(require) : i.createRequire("undefined" == typeof document ? require("url").pathToFileURL(__filename).href : o && o.src || new URL("dist/gql-tada-internal.js", document.baseURI).href).resolve;

var resolveExtend = async (e, n) => {
  try {
    return toTSConfigPath(L(e, {
      paths: [ n ]
    }));
  } catch (e) {
    return null;
...
var toTSConfigPath = e => ".json" !== s.extname(e) ? s.resolve(u, e, x) : s.resolve(u, e);
...
var loadConfigs = async e => {
  ...
  var visit = async (e, n) => {
    ...
    var l = await resolvePluginEntry(e, u);
    if (l) {
      var c = {
      ...
      if ((e => Array.isArray(e.references) && Array.isArray(e.files) && 0 === e.files.length && !e.include)(u)) {
        if (!i) {
          i = c;
        }
      } else {
        var f = `${toProjectKey(l.configPath)}\0${toProjectKey(c.rootPath)}`;
        if (!a.has(f)) {
          a.add(f);
          t.push(c);
        }
...
var _ = [ "/* eslint-disable */", "/* prettier-ignore */" ].join("\n") + "\n";
...
exports.outputIntrospectionFile = function outputIntrospectionFile(e, n) {
  ...
  if (/\.d\.ts$/.test(n.fileType)) {
    ...
    if ("string" != typeof e && n.shouldPreprocess) {
      a.push(`export type ${F} = ${preprocessIntrospectionTypes(e)};\n`, D, `export type introspection = ${preprocessIntrospection(e, F)};\n`, "import * as gqlTada from 'gql.tada';\n");
    } else {
      a.push(D, `export type introspection = ${stringifyJson(e)};\n`, "import * as gqlTada from 'gql.tada';\n");
    }
    if (!("name" in e) || !e.name) {
    ...
    return [ r, M, `const introspection = ${t} as const;\n`, "export { introspection };" ].join("\n");
  }
  ...
  for (var n of S) {
    ...
    for (var a of e) {
      ...
      for (var o of i) {
        ...
        var u = s.resolve(a.projectPath, d);
        if ("win32" === process.platform) {
          u = u.toLowerCase();
        }
        var l = r.get(u);
        if (l && l !== t) {
          throw new TadaError(`Projects '${l}' and '${t}' resolve '${n}' to the same file: ${d}`);
        }
        r.set(u, t);
      }
    }
