...
var sanitizeDocument = e => e.split(I).map(replaceOutsideStrings).join("").trim();
  ...
  for (var a of P) {
    ...
    for (var t of r) {
      ...
      for (var s of o) {
        ...
        var c = n.get(u);
        if (c && c !== i) {
          throw new TadaError(`Projects '${c}' and '${i}' resolve '${a}' to the same file: ${l}`);
        }
        n.set(u, i);
      }
    }
...
var q = "undefined" != typeof require ? require.resolve.bind(require) : h(import.meta.url).resolve;

var resolveExtend = async (e, r) => {
  try {
    return toTSConfigPath(q(e, {
      paths: [ r ]
    }));
  } catch (e) {
    return null;
...
var toTSConfigPath = r => ".json" !== e.extname(r) ? e.resolve(y, r, $) : e.resolve(y, r);
  ...
  var visit = async (r, a) => {
    ...
    if (Array.isArray(u.references)) {
      ...
      for (var p of u.references) {
        ...
        await visit((v = p.path, m = e.dirname(r), h = void 0, h = e.resolve(m, v), ".json" === e.extname(h) ? h : e.join(h, $)), !1);
      }
    }
    var v, m, h;
  };
...
function outputIntrospectionFile(e, r) {
  ...
  if (/\.d\.ts$/.test(r.fileType)) {
    ...
    if ("string" != typeof e && r.shouldPreprocess) {
      n.push(`export type ${G} = ${preprocessIntrospectionTypes(e)};\n`, V, `export type introspection = ${preprocessIntrospection(e, G)};\n`, "import * as gqlTada from 'gql.tada';\n");
    } else {
    ...
    return [ a, j, `const introspection = ${t} as const;\n`, "export { introspection };" ].join("\n");
  }
