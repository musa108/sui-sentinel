...
import { analyzeTransaction, TransactionData } from './aiService.js';
import { sendAlertNotification } from './notifier.js';
import { getCurrentPrice, getPriceVolatilityScore } from './priceFeeder.js';
import { executeEmergencyPause, isMarketPaused } from './autonomousAction.js';
 ...
 */
export async function processTransaction(tx: TransactionData) {
  // ---- 1. Load settings ----
  ...
  const priceVol = getPriceVolatilityScore(currentPrice);
  ...
  const finalRiskScore = Math.min(
    100,
    Math.round((aiResult.riskScore * 0.6) + (ruleScore * 0.4))
  );

  // Determine category from final score
  let finalCategory = 'Low Risk';
  if (finalRiskScore >= 85) finalCategory = 'Critical Risk';
  else if (finalRiskScore >= 60) finalCategory = 'High Risk';
  else if (finalRiskScore >= 35) finalCategory = 'Medium Risk';
  ...
  console.log(
    `[RiskEngine] ${tx.txHash.substring(0, 10)}... | ` +
  ...
  if (finalRiskScore >= 90 && !isMarketPaused()) {
    ...
    executeEmergencyPause(finalRiskScore).catch(err =>
      console.error('[RiskEngine] Autonomous pause failed:', err)
    );
  }
