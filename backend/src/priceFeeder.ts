...
export interface PricePoint {
  price: number;
  timestamp: number;
  change1m: number; // % change vs 1 minute ago
  change5m: number; // % change vs 5 minutes ago
}
...
// Module-level state
let currentPricePoint: PricePoint = {
  price: 3.45,
  timestamp: Date.now(),
  change1m: 0,
  change5m: 0,
...
export function getCurrentPrice(): PricePoint {
...
async function fetchPythPrice(): Promise<number | null> {
  try {
    ...
    if (price < 0.01 || price > 1000) return null;
    return price;
  } catch {
    return null;
  }
...
function pctChange(nowPrice: number, nowTs: number, ageMs: number): number {
 ...
 */
export async function startPriceFeeder(
  onPriceUpdate: (price: PricePoint) => void
): Promise<void> {
  ...
  async function poll() {
    ...
    if (price === null) price = simulatePrice();
    ...
    console.log(
      `[PriceFeeder][${source}] SUI/USD: $${price.toFixed(4)} | ` +
 ...
 */
export function getPriceVolatilityScore(price: PricePoint): VolatilityScore {
